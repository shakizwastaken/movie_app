...
const Movies = React.forwardRef(({searchInput,setMovies,movies},ref) => {
    ...
    const renderAllMovies = ()=>{
        return(movies.map(
            movie=><Movie key={movie.id} {...movie} onFav={toggleFav} />
        ));
    }
    ...
    const renderBySearch = ()=>{
        let result = movies.filter(
                movie=>getRelevancy(movie.name,searchInput) >=0
            )
        .sort(
            (movieA,movieB)=>getRelevancy(movieB.name,searchInput)-getRelevancy(movieA.name,searchInput)
        );
        return (
            result.length !==0 ?
            result.map(
    ...
    const renderByRating = (value)=>{
        return(
            movies.filter(
                movie=>movie.about.rating >=value
            )
            .map(
            movie=><Movie key={movie.id} {...movie} onFav={toggleFav} />
        ));
    ...
    const renderUserFav =()=>{
        return(movies.map(
            movie=>movie.isFav&&<Movie key={movie.id} {...movie} onFav={toggleFav} />
        ));
    }
    ...
    return (
        ...
        <>
            {searchInput && (<div ref={ref} className="movies-container">
